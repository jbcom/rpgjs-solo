...
const readNumber = (value: number | (() => number) | undefined, fallback = 0) =>
  typeof value === "function" ? value() : value ?? fallback;

const normalizeHitbox = (target: any) => {
...
const resolveAreaPoint = (center: MapAreaCenter): MapAreaPoint => {
...
const offsetCenter = (center: MapAreaPoint, offset?: AreaShapeOffset): MapAreaPoint => ({
...
const distance = (a: MapAreaPoint, b: MapAreaPoint): number =>
  Math.hypot(a.x - b.x, a.y - b.y);

const maxDistanceFromBounds = (
  center: MapAreaPoint,
...
const rotatePoint = (point: MapAreaPoint, angle: number): MapAreaPoint => {
...
const createFalloff = (distance: number, maxDistance: number): MapAreaFalloff => {
...
export abstract class RpgCommonMap<T extends RpgCommonPlayer> {
  ...
  queryArea<TCustom = unknown>(
    ...
    const ctx: MapAreaContext = { map: this, center };
    ...
    const excluded = new Set(options.excludeIds ?? []);
    const candidates = new Map<string, MapAreaCandidate<TCustom | T | any>>();
    ...
    if (kinds.has("players") || kinds.has("events")) {
      ...
      for (const target of this.queryHitbox(bounds, {
        excludeIds: options.excludeIds,
        kinds: hitboxKinds,
      })) {
        const id = (target as any)?.id;
        const kind: MapAreaTargetKind = id && this.players()[id] ? "players" : "events";
