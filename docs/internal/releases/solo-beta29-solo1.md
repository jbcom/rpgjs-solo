...
7. Run `pnpm publish:solo --manifest <absolute-provenance-path> --execute`; it distinguishes a verified registry package/version-not-found response (including a registry 404) from authentication, transport, and server errors and read-only preflights the complete cohort before local or remote mutation. It then descriptor-reads every source archive into a private mode-0700 transaction snapshot, binds that directory and every archive hash/integrity to a mode-0600 journal and ownership marker, and resumes an interrupted preparation only from proven state. Immediately before each `pnpm publish`, the mode-0400 snapshot archive is revalidated for type, ownership, mode, single-link count, SHA-512, and integrity. Only the private snapshot path is published; later source-path replacement cannot change the command input. Registry fetch-back remains the secondary post-publication proof.
