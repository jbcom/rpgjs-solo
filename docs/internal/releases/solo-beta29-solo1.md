...
runner boundary, and produced the signed candidate cohort. Release PR #24
merged the pnpm consumer-isolation correction. Corrective release PR #25 now
owns the Node/browser consumer-boundary correction and is the release PR bound
by the plan. Promotion remains blocked until that exact PR head passes every required
...
workspace and therefore left the intended consumer empty. PR #24 forced pnpm's
documented `--ignore-workspace` isolation and proved the hostile-parent case
directly. The isolated consumer then exposed a third boundary defect: the gate
loaded the browser-only renderer and CanvasEngine barrel in Node. PR #25 splits
that egress proof into a Node execution probe for transport-free packages and a
typed Vite browser build for renderer, test mute, CanvasEngine, and the required
fleet patch package. The already-published candidate archives are unchanged;
only a canonical passing gate may authorize promotion.
...
8. Run `pnpm release:solo:verify-candidate --manifest <absolute-provenance-path> --execute`; it verifies registry integrity and installs the exact cohort with `@arcade-cabinet/rpgjs-patches@0.2.0` in a fresh authenticated consumer. The install passes pnpm `--ignore-workspace`, so an unrelated `pnpm-workspace.yaml` in any parent of the operating-system temporary directory cannot capture or empty that consumer. The verifier then executes only the transport-free runtime, action-battle, and Vite inspection surfaces in Node. It separately runs strict TypeScript and a production Vite build over the renderer, runtime-only test mute, CanvasEngine factories, and fleet patch installer. A browser-only package imported by the Node probe, or a renderer integration that cannot compile and bundle for the browser, is a hard failure.
