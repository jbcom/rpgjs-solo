...
| `solo` | RPGJS Solo product | Reviewed Solo and deliberately ported upstream work |
| feature branches | One reviewable change | Focused work based on `solo` |

## Audit procedure

```bash
git fetch upstream v5
git fetch origin v5 solo
...
5. port or merge the selected changes to a feature branch based on `solo`;
...
Never resolve a sync by merging `solo` back into `v5`, force-pushing either
...
be replayed, dropped, or offered upstream independently.
