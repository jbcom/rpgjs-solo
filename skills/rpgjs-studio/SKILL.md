...
- Maps may expose `mapLoadBlockCollectionId: string | null`. When set, `GET /api/game/maps/:mapId` hydrates that collection into `mapLoadBlocks`, and the RPGJS Studio runtime executes those blocks once from the server `map.onLoad(map)` hook. This is a map-level workflow without a guaranteed player or event context.
